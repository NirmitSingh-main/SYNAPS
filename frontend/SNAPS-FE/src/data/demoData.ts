...
export const demoAnalysis: DemoAnalysis = {
  ...
  features: [
    {
      ...
      value: "20.6",
      unit: "kHz",
      description: "Mean instantaneous frequency deviation",
    },
    {
    ...
    },
    {
      ...
      value: "0.025",
      unit: "",
      description: "4th-order cumulant for constellation symmetry",
    },
    {
      name: "Higher-Order Cumulant C42",
      value: "0.435",
      unit: "",
      description: "4th-order cumulant for power variance",
    },
    {
      name: "Peak-to-Average Power Ratio",
      value: "5.4",
      unit: "dB",
      description: "Crest factor of the signal envelope",
    },
  ],

  // ---- Illustrative explanation ----
  explanation:
    "The extracted Higher-Order Cumulants and narrow spectral profile are consistent with a BPSK modulated carrier. The neural Transformer model classified this signal with 91.3% confidence, corroborated by DSP cumulant analysis.",
  ...
  waveformSamples: Array.from(
    ...
    (_, i) => {
      ...
      return (
        Math.sin(
          t * Math.PI * 2 * 12 +
            Math.sin(t * Math.PI * 2 * 1.5) * 3.2
        ) *
          0.7 +
        Math.sin(t * Math.PI * 2 * 37 + 0.4) *
          0.12 +
  ...
  spectrumBins: Array.from(
    { length: 64 },
    (_, i) => {
      const x = i / 64;

      return (
        Math.exp(-Math.pow((x - 0.22) * 6, 2)) *
          0.92 +
        Math.exp(-Math.pow((x - 0.62) * 9, 2)) *
          0.54 +
        Math.max(
          0,
          0.08 - Math.abs(x - 0.5) * 0.3
        ) +
